...
SELECT ProductName, char_length(ProductName), floor(UnitPrice), UnitsInStock, round(UnitsInStock*UnitPrice) as "Ganancia" FROM northwind.products;
...
-- con subconsultas
Select count(*) "cantidad de productos" from products where CategoryID = (SELECT CategoryID FROM categories where CategoryName = "Beverages");

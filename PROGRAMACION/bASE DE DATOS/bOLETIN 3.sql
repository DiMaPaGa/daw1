-- Mostrar todos los empleados y todos sus atributos.
-- SELECT * FROM northwind.employees;
-- Mostrar todos los productos y todos sus atributos.
-- SELECT * FROM northwind.products;
-- Mostrar los productos que pertenecen a la categoría 2.
-- SELECT * FROM northwind.products WHERE CategoryID=2;
-- Productos que pertenece a la categoría 2 y 4.
-- SELECT * FROM northwind.products WHERE CategoryID=2 and CategoryID=4;
-- Productos que pertenece a la categoría 2 o 4.
-- SELECT * FROM northwind.products WHERE CategoryID=2 or CategoryID=4;
-- Mostrar id de producto, nombre de producto y precio unitario para todos aquellos
...
-- Mostrar el producto con nombre “tofu”.
-- SELECT * FROM northwind.products WHERE ProductName = "tofu";
-- Mostrar los productos cuyo ID se encuentra en la categoría, 2, 3 o 4. Hacer de dos formas distintas.
...
-- Mostrar los productos cuyo ID de proveedor se encuentra en la categoría 3,5 u 8.
-- SELECT * FROM northwind.products WHERE  SupplierID IN (3,5,8);
-- Mostrar los productos cuyo precio oscilan entre 30 y 60 dólares.
-- SELECT * FROM northwind.products WHERE  UnitPrice BETWEEN 30 AND 60;
-- Mostrar los productos cuyo stock está entre 0 y 10.
-- SELECT * FROM northwind.products WHERE UnitsInStock BETWEEN 0 AND 10;
-- Mostrar todos los productos que empiezan con ‘c’.
-- SELECT * FROM northwind.products WHERE ProductName LIKE"c%";
-- Ordenar todos los productos por nombre, en orden ascendente.
-- SELECT * FROM northwind.products ORDER BY ProductName ASC;
-- Ordenar todos los productos por ID, en orden descendente.
...
-- Obtener todos los clientes de los países de: USA, Francia o UK. Hacer de dos formas.
...
-- Obtener todos los productos descontinuados (Discontinued=1) o sin stock, que pertenecen a la categoría 1, 3, 4 o 7.
-- SELECT * FROM northwind.products WHERE Discontinued=1 AND CategoryID IN (1,3,4,7);
-- Obtener todas las órdenes hechas por el empleado con código: 2, 5 o 7 en el año 1996.
-- SELECT * FROM northwind.orders WHERE EmployeeID IN (2,5,7) AND OrderDate LIKE ("1996%");
-- Seleccionar todos los clientes que cuenten con Fax (valor distinto de null).
-- SELECT * FROM northwind.customers WHERE Fax IS NOT NULL;
-- Seleccionar todos los empleados que cuentan con un jefe (campo ReportsTo).
-- 
-- Seleccionar todos los campos del cliente, cuya compañía empiecen con letra A hasta la D y pertenezcan al país de USA, ordenarlos por la dirección.
-- 
-- Seleccionar todos los campos del cliente, cuya compañía empiecen con las letras de la B a la G, y pertenezcan al país de UK, ordenarlos por nombre de la compañía
-- 
-- Seleccionar los productos vigentes cuyos precios unitarios están entre 35 y 250, sin stock en almacén, pertenecientes a las categorías 1, 3, 4, 7 u 8, que son
-- distribuidos por los proveedores, 2, 4, 6, 7, 8 o 9
-- 
-- Seleccionar todos los campos de los productos descontinuados, que pertenezcan a los proveedores con códigos: 1, 3, 7, 8 y 9, que tengan stock en almacén, y al
